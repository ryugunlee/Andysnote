...
- **Local** (`local.js`): BROWSER-based file I/O, NOT the real OS filesystem. Primary store = IndexedDB (`andysnotes-local` db, `notes` store, keyPath `id`; note shape `{id,title,body,createdTime,modifiedTime}`, ids `"local-"+Date.now()+"-"+rand`). Shown in its own sidebar section **notes_local** (`#local-list`), an app-managed list — NOT a mirror of any folder. The OS filesystem is only touched through explicit user-driven `.txt` **export** (`showSaveFilePicker` → `<a download>` fallback) and **import** (`showOpenFilePicker` → hidden `<input type=file>` fallback).
